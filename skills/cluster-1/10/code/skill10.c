//John Kircher 09/20/2020
//Attributions to the i2c example code, blink example code and design pattern RTOS TASK on class site
//Attributions also to team-mate Zhangchi.
...
static void IRAM_ATTR gpio_isr_handler(void* arg){  // Interrupt handler
    flag ^= 1;// Toggle state of flag
}

static void init(){
	// Initialize Button Borrowed from classmate
	gpio_pad_select_gpio(15);
	    gpio_set_direction(15, GPIO_MODE_INPUT);
	// Initialize LED
	int BLINK_GPIO[4] = {12,4,25,26};
    ...
    while(1) {
     ...
     led[3] = ((!carry[2])&led[3])|(carry[2]&(!led[3]));
        ...
        vTaskDelay(1000 / portTICK_PERIOD_MS);
    		...
    		led[1] = ((!carry[0])&led[1])|(carry[0]&(!led[1]));
    		carry[2] = (!led[2])&carry[1];
    		led[2] = ((!carry[1])&led[2])|(carry[1]&(!led[2]));
    		led[3] = ((!carry[2])&led[3])|(carry[2]&(!led[3]));
    		gpio_set_level(BLINK_GPIO[0], !led[0]);
    		        gpio_set_level(BLINK_GPIO[1], !led[1]);
    		        gpio_set_level(BLINK_GPIO[2], !led[2]);
...
static void taskC() {
    ...
    //char cheat sheet
    static const uint16_t alphafonttable[94] ={
    		0b0000000000000000, //
    	    0b0000000000000110, // !
    	    0b0000001000100000, // "
    	    0b0001001011001110, // #
    ...
    uint16_t displaybuffer[4];
    	...
    	for (int i = 0; i < 4;i++){
    		for (int j = 0;j<94;j++){
    			if (str[i] == alphatable[j])
    				displaybuffer[i] = alphafonttable[j];
    		}
    	}
...
void app_main() {
    init();
    i2c_scanner();
    xTaskCreate(taskA, "taskA", 2048, NULL, configMAX_PRIORITIES, NULL);
    xTaskCreate(taskC, "taskC", 4096, NULL, configMAX_PRIORITIES-1, NULL);
    xTaskCreate(taskB,"taskB", 2048, NULL,  configMAX_PRIORITIES-2, NULL);
}
